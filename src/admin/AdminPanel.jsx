    ...
    <>
      ...
      <div className={`admin-panel admin-panel--open fixed top-0 right-0 h-full w-80 bg-gray-900 text-white z-50 flex flex-col shadow-[-4px_0_24px_rgba(0,0,0,0.3)] border-l border-gray-700`}>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700">
          ...
          <button
            onClick={handleClose}
            className="w-7 h-7 flex items-center justify-center rounded hover:bg-gray-800 text-gray-400 hover:text-white transition-colors border-0 bg-transparent"
          >
        ...
        <div className="px-4 py-3 border-t border-gray-700 flex gap-2">
          <button
            onClick={resetConfig}
            className="flex-1 px-3 py-2 text-xs font-medium bg-red-900/40 hover:bg-red-900/60 text-red-300 rounded-lg transition-colors border-0 cursor-pointer"
          >
            Reset
          </button>
          <button
            onClick={handleExport}
            className="flex-1 px-3 py-2 text-xs font-medium bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg transition-colors border-0 cursor-pointer"
          >
            Export
          </button>
          <button
  ...
  return (
    <>
      <SelectControl
        ...
        onChange={v => updateConfig('typography.fontFamily', v)}
        options={FONT_OPTIONS}
      />
      <SliderControl
        label="Hero Headline"
        value={config.typography.headlineSize}
        onChange={v => updateConfig('typography.headlineSize', v)}
        min={1.5} max={4} step={0.125} unit="rem"
      />
      <SliderControl
        label="Form Title"
        value={config.typography.formTitleSize}
        onChange={v => updateConfig('typography.formTitleSize', v)}
        min={1} max={3} step={0.125} unit="rem"
      />
      <SliderControl
        label="Body Text"
        value={config.typography.bodySize}
        onChange={v => updateConfig('typography.bodySize', v)}
        min={0.75} max={1.5} step={0.0625} unit="rem"
      />
      <SelectControl
        label="Headline Weight"
        value={config.typography.headlineWeight}
        onChange={v => updateConfig('typography.headlineWeight', v)}
...
function ContentTab({ config, updateConfig }) {
  return (
    <>
      ...
      <TextControl label="Logo Text" value={config.content.logoText} onChange={v => updateConfig('content.logoText', v)} />
      <TextControl label="Logo Letter" value={config.content.logoLetter} onChange={v => updateConfig('content.logoLetter', v)} maxLength={2} />

      <SectionLabel>Hero</SectionLabel>
      <TextControl label="Headline" value={config.content.headline} onChange={v => updateConfig('content.headline', v)} multiline />
        ...
        <div key={i} className="space-y-3 pb-4 border-b border-gray-800 last:border-0">
          <div className="flex items-center justify-between">
            ...
            <ToggleControl
              ...
              onChange={v => updateConfig(`cards.${i}.visible`, v)}
            />
          </div>
          ...
          <TextControl label="Image URL" value={card.image} onChange={v => updateConfig(`cards.${i}.image`, v)} placeholder="https://..." />
        </div>
      ))}
    </>
  )
  ...
  return (
    <>
      ...
      <SliderControl
        ...
        onChange={v => updateConfig('layout.borderRadius', v)}
